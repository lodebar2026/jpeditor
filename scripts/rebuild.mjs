...
import { loadCorpus, gtKeyTime, loadCli, songRank } from "./node-harness.mjs";
import { writePdf } from "./pdfwrite.mjs";
import { makeMetrics } from "./textmetrics.mjs";
import { openDb, readTable } from "./checkdb.mjs";
...
if ("db" in flags) {
  const { newRunId, recordRun, recordMetrics } = await import("./checkdb.mjs");
  const db = openDb();
