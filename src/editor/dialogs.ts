...
import { JIANPU_PAPERS, PAGE_RATIOS, PAPER_SIZES, PPT_PAPERS } from "./app";
...
/** 选项。**只摆当前模式下真正生效的项**——面板是全局的，但四档视图（PPT / 简谱 /
...
export function showOptionsDialog(app: App): void {
  ...
  /** 简谱排版器那条路（PPT 档或简谱档）——下面绝大多数项只有它吃。 */
  const isJp = !isPu && !isMixed;
  const isPpt = isJp && view === "ppt";
  const isJianpu = isJp && view === "jianpu";
  ...
  // ---- 纸张 ----
  // PPT 档取 PAGE_RATIOS 的**绝对尺寸**（导出 PPTX 就是这个纸）；简谱档只取**比例**，
  // 纸宽锁定（见 App.layoutPage），所以两档虽然共用这张表，选中项各按各的算。
  /** 纸张下拉。`withSize` 时把实际尺寸写进选项文字——简谱档选的是**纸**（「A4」不该还要人
   *  去猜多大）；PPT 档选的是**比例**（「16:9」本身就说清楚了），照旧只写键名。
  ...
  // PPT 档是投影片，选的是 16:9 / 4:3 那几个比例；简谱档选的是实际纸张，外加一档长图。
  ...
  const jpPaper = paperSelect(JIANPU_PAPERS, (k) => PAPER_SIZES[k], (k) => k === app.jpPaper, true);
  ...
  // 「每页行数」只归 PPT 档：那一档是逐段展开、一屏一段，每页放几行是版面决定；
  // 简谱档按原谱排一遍，行数由内容与纸说了算，人为定死只会把谱挤坏。
  ...
  // 文本谱两档都能选纸：两档的分野只是**展开反复与否**，纸张是独立设置项。
  // PPT 那份没有长图（PPT 恒分页），简谱那份没有投影片。
  const puIsPpt = isPu && view === "ppt";
  const puPaper = paperSelect(
    puIsPpt ? PPT_PAPERS : JIANPU_PAPERS,
    (k) => PAPER_SIZES[k],
    (k) => k === (puIsPpt ? app.puPptPaper : app.puPaper),
    true,
  );
  // 字号留空/0 = 跟随版式量到的原尺寸；有排好的谱就把当前实际字号填进去当起点
  const puFont = num(
    (puIsPpt ? app.puPptFontSize : app.puFontSize) || Math.round(app.puPainter?.digitFontSize ?? 0),
    6, 200,
  );
  ...
  if (isJp) {
    ...
    // 简谱档只调基础字号：那一档的标题与词曲字号是按比例派生的（App._setJpFontSize），
    // 摆出来只会让人以为能单独调。PPT 档三个都是独立设置，照旧全给。
  ...
  modal("设置", body, () => {
    ...
    app.applyRenderSettings({
      ...
      puPptPaper: puIsPpt ? puPaper.value : undefined,
      puFontSize: isPu && !puIsPpt ? parseInt(puFont.value, 10) || 0 : undefined,
      puPptFontSize: puIsPpt ? parseInt(puFont.value, 10) || 0 : undefined,
