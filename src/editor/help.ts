...
const FEATURE_TOPICS: Topic[] = [
  {
    ...
    body: [
      ...
      "右上角 **保存** 把当前简谱存成 `.jpwabc`（UTF-16LE 编码，与 JP-Word 兼容）；`Ctrl/⌘+S` 保存，`Ctrl/⌘+Shift+S` 另存为；**导出** 用于生成 PDF、PNG、MIDI 或 PPTX。",
      "**桌面版**：第一次保存选择路径，之后 `Ctrl/⌘+S` 会原地写回当前文件；下次启动会自动恢复上次打开的文件。**浏览器版**：用网页文件选择器打开，保存则以下载方式导出。",
    ],
  },
  {
    title: "编辑与实时排版",
    body: [
      ...
      "源码栏右上角显示当前**行、列和修改状态**；光标落在音符或歌词时，右侧谱面会出现绿色编辑光标，直接指出正在修改的位置。",
  ...
  },
  {
    ...
    body: [
      ...
      "**MusicXML**（`.xml` / `.musicxml`）：导入后若是多声部（如四部合唱）会自动进入**五线谱**预览。",
    ],
  },
  {
    title: "谱面布局",
    body: [
      "右侧预览顶部的 **简谱 / 五线谱**：导入图片、MusicXML 或 ABC 后可用，用于切换同一识别结果的两种预览；五线谱由 MusicXML 渲染，此模式下左侧代码只读。",
      "**分行方式**：选择「原始排版」可保留导入或识别时的行结构；选择「按乐句重排」会根据歌词和音乐乐句重新安排换行。",
      "这些工具只会在当前文件支持时出现。",
    ],
  },
  {
    title: "播放",
    body: [
      "工具栏 **试听** 按钮会随状态显示 播放 / 加载中 / 停止；播放时当前发声的音符会高亮。若先点选了某个音符，则从该处开始播放。",
      "多声部时可在 **设置** 里调各声部音量。",
  ...
  },
  {
    ...
    body: [
      ...
      "五线谱模式可导出当前页 **PNG**、全部页面 **PDF** 和 **MIDI**，均直接下载。导出内容会与当前五线谱预览及「显示简谱层」设置保持一致。",
    ],
  },
  {
    title: "简繁转换",
  ...
  },
  {
    ...
    body: [
      ...
      "多声部简谱还有各声部音量；五线谱模式下可勾选「隐藏小节号」，并可在预览区顶部通过「显示简谱层」决定是否保留数字简谱信息。",
    ],
  },
  {
    title: "键盘快捷键",
