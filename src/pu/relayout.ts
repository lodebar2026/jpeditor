...
// 和弦 `"hx:…"`、`&记号`、注释、并字号、跳词符都跟着原文走。简谱引擎输入不带的那些东西
// 因此一个都不会掉。
//
// 为什么落到文本而不是往引擎输入插换行：原样档走的是 `PuPainter`（吃排版行视图，不经引擎输入），
// 那一层的断点够不着它。而文本谱里「一行 `Q:` 就是谱面一行」，重切行两档同时就位——
// 展开档那边行边界照旧由 `model/jianpuinput.ts` 转成换行条目。
//
