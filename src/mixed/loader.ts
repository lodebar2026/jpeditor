...
function layoutInputOf(partEls: Element[]): LayoutInput {
  return partEls.map((pt) =>
    elems(pt, "measure").map((mea) => ({
      prints: elems(mea, "print").map((pr) => {
        const sl = elem(pr, "system-layout");
        const sm = sl ? elem(sl, "system-margins") : null;
        return {
          newPage: pr.getAttribute("new-page") === "yes",
          newSystem: pr.getAttribute("new-system") === "yes",
          systemLayout: sl
            ? {
                margins: sm ? { left: floatOf(sm, "left-margin"), right: floatOf(sm, "right-margin") } : null,
                topSystemDistance: floatOf(sl, "top-system-distance"),
                systemDistance: floatOf(sl, "system-distance"),
              }
            : null,
          staffLayouts: elems(pr, "staff-layout").map((s) => ({
            number: attrInt(s, "number") ?? 1,
            staffDistance: floatOf(s, "staff-distance"),
          })),
        };
      }),
      staffDetails: elems(mea, "attributes").flatMap((at) =>
        elems(at, "staff-details").map((det) => ({
          number: attrInt(det, "number") ?? 1,
          printObject: det.getAttribute("print-object"),
        })),
      ),
    })),
  );
...
function loadPartGroups(score: MixedScore, partListEl: Element): PartGroup[] {
...
export function loadMixedXml(xmlText: string, options: MixedOptions): MixedScore {
  ...
  finishMixedScore(score, layoutInputOf(partEls), partListEl ? loadPartGroups(score, partListEl) : []);
