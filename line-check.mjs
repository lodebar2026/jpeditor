...
for (const song of lineDoc.songs) {
  ...
  ls.forEach((l, i) => {
    ...
    //     （用户口径 2026-08-28：「避免残小节全是休止，不管是在行首还是行尾」；J09《祢的话》
    //     第 4 行末尾的 `|]0`）。D1 是它在行首那一半。修在 `applybreaks.ts::tidyLineTails`。
    //     **上一句的收气休止不算**（与 D5 是一体两面）：`|5-多 5-深？|0 3主 1恩 3有|` 那个
    //     `0` 是上一句唱完的那口气，留在行末才对——139《主爱有多少》原书四行行末就都是 `|0`。
    //     判据照 D5 反着来：上一句在这里收了句，而且把这一截推到下一行会让那行的弱起
    //     与本曲对不上，就是这一类，豁免。
    if (i < ls.length - 1 && l.tail.restOnlyBar) {
      const pushed = l.tail.restBarDur + (ls[i + 1]?.head.dur ?? 0);
      const breath = l.tail.lastWordPunct > 0 && !(pickup > 0 && Math.abs(pushed - pickup) < 0.01);
      //     **下一行已经满了的也豁免**：推过去它就放不下，那是版心的账不是断句的
      //     （396《生命在此》第 5 行 308.7 / 版心 311.9）。这一截有多宽按本行的平均宽度估
      //     ——真判在 `tidyLineTails` 里按真实坐标做，这里只是别误报。
      const barW = l.dur > 0 ? (l.width * l.tail.restBarDur) / l.dur : 0;
      const noRoom = (ls[i + 1]?.width ?? 0) + barW >= (song.width ?? Infinity);
      if (!breath && !noRoom)
        hit("D10", song.id, `第 ${i + 1} 行行末挂着 ${l.tail.restBarDur} 拍的空小节（收在「${l.tail.lastWord}」）`);
    }
 ...
 *  钉成快照等于把那条阈值也钉死了。
 *  **020《向主歌唱》摘掉**（用户口径，2026-09-01：「两种方案都可以接受」）：末两行
 *  12.5 / 14.5 与 15.5 / 11.5 之差——原书那首末行只有 3 小节、上一行 6 小节，
 *  后者反倒更贴原书。它的 D1 / D8 仍在下面的豁免名单里，两档判据照旧守着。 */
const SNAPSHOT_IDS = [
  "061", "068", "070", "077", "095", "096", "103", "118",
  "120", "175", "283", "312", "355", "363", "374", "378",
  "446", "459", "490",
];
/**
