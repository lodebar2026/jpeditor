...
| ~~`MixedScore`~~ | 原先五线谱**语义 + 排版**混在一起（tenths） | **已删**（R2 阶段 10）：五线谱引擎只留版面态 `StaffLayout`（`src/mixed/model.ts`），版面节点带 `src` 引用 `ScoreDoc` 元素，语义一律从它取；建版在 `mixed/layout.ts::layoutStaff` |
