# jpeditor-web 架构与实现笔记

> 这份是**细节全录**：架构决策的原委、各模块的判据与踩坑记录。
> 根目录的 `CLAUDE.md` 是它的简版索引，改到哪一块再回这里翻哪一节。
...
npm run build && node scripts/shot.mjs /tmp/out.png            # 截 #score-pane + 诊断
npm run build && node scripts/pptx-check.mjs                   # 两档版面：原样档叠排多段 + 展开档笔画
npm run build && node scripts/page-check.mjs [--write-baseline] # 分页版面：P1 越纸 / P2 谱行相压 / P3 压页脚 / P4 行内歌词相压 / P5 展开档结构（基线 testdata/page-check-baseline.json）
npm run build && node scripts/pptx-ref-check.mjs               # 导出的 .pptx 成品几何 ↔ ppt500 那批成品
npm run build && node scripts/pptx-export.mjs [--out= --one= --limit=]  # 500 首 musicxml → 逐首 .pptx
npm run build && node scripts/zoom-check.mjs                    # 纸张宽度四档一致 + 双指缩放/平移
npm run build && node scripts/abc-check.mjs                    # ABC→MusicXML 移植回归（见 docs/实现/ABC-导入.md）
npm run build && node scripts/xml-roundtrip.mjs                # MusicXML 导出回归（序列化往返 / 增量 patch / 版面）
npm run build && node scripts/omr-export-check.mjs             # 同上，但底本取自真跑一遍 OMR 的识别原文
npm run build && node scripts/abc-shot.mjs <abc> /tmp/abc.png  # 拖入 .abc 端到端渲染核对
npm run build && node scripts/omr-pu-check.mjs                 # 识别→文本谱原文→回解析 逐项对拍
npm run build && node scripts/pu-phrase-check.mjs              # 文本谱按乐句重排：音符/歌词对位等价 + 诊断不增 + 幂等
npm run build:cli && node scripts/omr-cli.mjs <图…|目录…> [-f shige|tomato|jpwabc] [-o 出] [--profile] [--thread-mode=auto|always|single]  # 简谱 OMR 命令行（不起浏览器；支持批量）
npm run build && npm run build:cli && node scripts/omr-node-check.mjs  # Node 管线 ↔ 浏览器管线 逐字符一致性（Node CLI 的验收依据）
npm run build:cli && node scripts/pack-omr.mjs [--targets=all|linux-x64,…] [--libc=musl] [--no-slim]  # OMR CLI 自包含分发包（交叉打包 mac/linux/win）
npm run build:cli && node scripts/pdf-diff.mjs && node scripts/pdf-mark.mjs   # 矢量 PDF 识别 ↔ GT 对比 + 差异标记版 PDF
node scripts/rebuild.mjs [--one=020,373] && node scripts/line-check.mjs        # 成书重排 + 判据断言（断句 D1~D10 / 版面 V1~V11 + 全书基线）
node scripts/gen-storyocr.mjs && node scripts/gen-glyphsheet.mjs               # 整行 OCR 补字（两引擎投票）+ 人工确认表
swiftc -O -o tools/vision-ocr tools/vision-ocr.swift -framework Vision -framework PDFKit  # Apple Vision 那一路
node scripts/gen-glyphaudit.mjs [祂 …]                                 # 同字多形审计：捞原书错字/错标（→ glyph_fix）
node scripts/gen-annocheck.mjs [--issues] [--one=022]                  # 注解对比册：原件裁图 ↕ 识别文本（缺字画 □）
node scripts/gen-glyphfuzzy.mjs [--dry] [--why]                        # 形近补字：签名匹配回字典已认得的类 + 上下文规则
node scripts/gen-bookmeta.mjs [--check]                                # 书级内容（调号拍号/目录/索引/注解…）入 校对.db
node scripts/gen-glyphmerge.mjs        # 字形建库第三步：同一字形的分身归并、标注取齐
npm run build && node scripts/gen-pu-gt.mjs                    # 生成和弦 GT 底稿（须人工核对后才算 GT）
npm run build:cli && node scripts/gen-staffglyphs.mjs          # 五线谱：建字形库 + 人工确认表
node scripts/gen-stafflyrics.mjs                               # 五线谱：拿 GT 自举正文字形字典（修乱码 CJK）
npm run build && node scripts/gen-staffocr.mjs                 # 五线谱：未定字形送 OCR 兜底（与上一步交替跑）
npm run build:cli && node scripts/gen-staffmasks.mjs            # 五线谱：贴图字（JBIG2 位图当字用）建库
node scripts/staff-report.mjs && node scripts/staff-diff.mjs           # 五线谱：逐页事实 + 逐首对拍（基线断言）
node scripts/staff-diff.mjs --errors                                   # 五线谱：逐条错误清单 → staff-out/staff-errors.json
node scripts/gen-staff-analysis.mjs <曲名> --zmzq                       # 五线谱：逐曲诊断报告（HTML，先跑上一行）
node scripts/staff-crop.mjs 110 40,360,580,400 /tmp/a.png      # 五线谱：按识别用的坐标裁一块原件出来看
node scripts/staff-export.mjs <页范围> out.musicxml            # 五线谱：识别 → MusicXML
npm run build && node scripts/staff-ui-check.mjs [页号]        # 五线谱：编辑器接线的端到端检查

npm run build:cli && node scripts/gen-rasterglyphs.mjs         # 位图五线谱：建形状字典 + 人工确认表
node scripts/chorus-report.mjs                                 # 位图五线谱：逐页事实（形态/线距/线宽/倾斜）
node scripts/chorus-diff.mjs [--one=宁静] [--v] [--bless] [--errors] # 位图五线谱：逐声部对拍（基线断言）；--errors 出逐条错误清单
node scripts/raster-gap.mjs [--one=破碎]                       # 位图五线谱：漏掉的音「墨归了谁、那团多大」（先跑 --errors）
node scripts/chorus-symbols.mjs --errors [--scan]              # 位图五线谱：逐条符号错例（谱号/时值/附点/升降/弧/松叶/力度）
node scripts/gen-staff-analysis.mjs 宁静 [--pdf=1] [--max=12]  # 位图五线谱：逐曲诊断报告（HTML；九类错例 + 中间过程 + 错因归因，先跑上面两条 --errors）
npm run build && npm run build:cli && node scripts/gen-rasterlyrics.mjs [--scan] # 位图五线谱：歌词条 OCR 缓存；--scan 补扫描件
npm run build && npm run build:cli && node scripts/gen-rasterlabels.mjs [--one=破碎] # 位图五线谱：声部标签 OCR 缓存（给跨系统全局指派定身份）
node scripts/chorus-symbols.mjs [--one=宁静] [--v] [--scan]     # 位图五线谱：逐符号及坐标错例；--scan 评测扫描件
node scripts/raster-unclaimed.mjs [--one=宁静] [--top=25] [--scan] # 位图五线谱：无主 contour 报表（还有什么是从没看见的）
node scripts/raster-lines.mjs [--one=差遣]                      # 位图五线谱：谱线漏检自检（行投影 ↔ 逐列游程）
node scripts/raster-prims-check.mjs                            # 位图五线谱：粗加线、弯曲擦线、推平与符干保护回归
node scripts/xml-lyrics-check.mjs                              # MusicXML 歌词：数字/Sibelius 段号与非歌词标签回归
node scripts/raster-crop.mjs 宁静 1 /tmp/a.pgm --staff=4        # 位图五线谱：裁图并把识别结果画上去
```

`scripts/shot.mjs` 用 Playwright `channel: "msedge"` 驱动本地 Edge，serve `dist/`，加载后截图并
...
（附点的**圆心与数字墨迹的中心等高**，大小默认与八度点相同——`augDotRadius` 只有展开档另给），
...
- `src/jianpu/` — 简谱排版两种格式（`.jpwabc` / 文本谱）的**公共那一半**：`profile.ts` 档位、
  `expanded.ts` 展开档排版器 `ExpandedPainter`（两种格式共用，吃 Score）、`vertical.ts` 纵向分页、
  `expand.ts` 反复展开的逐遍走法与换页口径。详见下文「展开 / 原样两档」一条。
...
- **矢量 PDF 路**（**目前只有 CLI 脚本走这条路，编辑器尚未接**——`isVectorPdf` 已备好
  ...
  与起 Edge 的 `scripts/harness.mjs` 分工）。详见 [矢量PDF识别](实现/矢量PDF识别.md)。
...
- **设置面板只摆当前模式下真正生效的项**（`editor/dialogs.ts::showOptionsDialog`）。面板是全局的，
  但四档各走各的排版器，参数并不通用：字号与前景色只有简谱排版器与文本谱排版器吃
  （混排用 MusicXML 自己那套），纸张与字号两档各记各的，
  「每页行数」（写进文档 `.Layout` 段）**只归展开档**——那一档逐段展开、一屏一段，每页放几行是版面决定；
  原样档按原谱排一遍，行数由内容与纸说了算，人为定死只会把谱挤坏。**没摆出来的项一律不回灌**：
  把控件初值当用户输入送回 `applyRenderSettings`，等于替用户做了他没做过的决定。
  背景色是唯一四档通吃的——它铺的是**纸**不是谱（`App._applyPageBg` 把颜色灌成 `#score-pane` 上的
  `--score-page-bg`，由页元素继承；导出时 PNG 铺画布底、混排 PDF 往 SVG 底层插满幅 rect、
  PPTX 写 `<p:bg>`，白底一律不铺）。
- **文本谱也能手动设**（`pu/metrics.ts::applyUserOptions`）：谱面自带的 `FontSize:`/`Margin:` 先生效，
  面板那一层叠在最外面。字号面板上给的是 **pt**（与简谱同一量纲），但文本谱的尺寸是从原书逐项量出来的
  一整套、不由一个基础字号派生，所以 `PuPainter.resolveScale` 把它换算成**整体缩放**：
  分母是「这套版式（含谱面自带的 `FontSize` 指令）原本多少 pt」，由 `digitFontSizeOf` 从
  `digitInkHeight` 反推——那个函数因此提到了模块级，metrics 定稿之前就得算得出来。
  纸与页边距不跟着缩，版心不变、字大了每行就放得少；填 0 = 跟随版式量到的原尺寸；
  换纸给的是实际尺寸（与 `.jpwabc` 原样档共用 `PAPER_SIZES`）。**这些只归文本谱的原样档**（`puPaper`/`puFontSize`，
  文本谱原样档的字号只有「基础字号」一项）；**展开档两种格式是同一个排版器，设置也是同一份**
  （比例 `pageW/pageH` + `_sizes.pptx` 三个字号 + 展开档配色，由 `App.expandedOptions` 组一次），面板上摆的也是同一组。
  前景色走 `pu/painter.ts::setPuInk`——颜色出现在那个文件几十处画笔的默认参数里，
  逐处穿参数等于把颜色织进每个绘制助手的签名，所以是**模块级可变**、`load` 开头设一次
  （排版是同步的，同一时刻只有一个 PuPainter 在跑）。
- **「展开」/「原样」两档的唯一语义是「排版方式」**，不是版式主题、也不是文档格式的分支：
  **展开 = 反复与多段歌词逐遍展开**（一段歌词一遍谱、逐遍成页，投影时一屏一段）、
  **原样 = 按原谱排一遍**（多段歌词叠在同一条谱行下，反复不展开）。
  **`.jpwabc` 与文本谱（`.pu`）两种格式语义完全一致**，公共的那一半在 `src/jianpu/`：
  `profile.ts`（档位 `JianpuLayoutMode`；`jpProfile` / `puProfile` 只是两种格式各记的档名）、
  `expanded.ts`（**展开档排版器 `ExpandedPainter`，两种格式共用一个**：输入是一份 `Score` + `ExpandedOptions`，
  格式只是数据来源——`.jpwabc` 经 `jpwimport`、文本谱经 `pu/toscore.ts`；笔画走 `applyPptxStyle`，
  第一页独立的词曲标题页、其后每页页脚曲名 + 页码 `0.8 × 纸宽`）、
  `vertical.ts`（纵向分页 `paginate`：只认块的占位盒，两格式共用一份）、
  `expand.ts`（`walkPlay` 按 `PlayItem[]` 逐遍逐小节回调；原样档走 `identityPlan`）。
  「这首歌唱成什么样」一律由 `Score.parseRepeatInf()` 推（文本谱先由 `pu/toscore.ts` 补上房号与跳转记号），
  展开档「怎么放一个小节」就是 `layout.ts::buildLine` 往 `Line` 里装。
  文本谱展开档**只排一条旋律**（Layout 只排 `parts[0]`）：`puToScore({ forExpanded })` 把带歌词的声部换到最前
  （合唱谱的词常挂在 Q2）、同一曲行下段号重复的歌词行顺延成下一段（一遍只挂一行词）；和弦进不了 Score、首调号拍号展开档本就不画，所以这两样自然不出现。
  原样档各排各的：`.jpwabc` 是 `layout/painter.ts::JinpuPainter`，文本谱是 `pu/painter.ts::PuPainter`（印刷原版观感）；
  两者与 `ExpandedPainter` 的 Layout 那一半（页面、标题页、点选、高亮索引）在 `layout/painter.ts::ScorePainter`。
  纸张、字号、配色**不属于这两档的分野**——它们是独立的设置项，两档各记各的一套。
  **换页口径只在 `walkPlay` 里判**：`endOfPass`（`.Repeat` 的 `P`、按段数逐遍）与**往回跳**
  （反复、D.C./D.S.）处换页；遍号变了但往前接着唱的（二房后的尾段）不换。源里的强制分页（`$` 分页）另算。
- **谱面区的四档排版模式**（`app.ts::ViewMode`，工具条上叫「排版」）是**两组正交状态的组合**，
  不是四个视图：`展开` / `原样` 走简谱（或文本谱）排版器，差别只在上面那一条
  （`setProfile` → `jpProfile` normal/pptx、`puProfile` print/slide）；`五线谱` / `混排`
  走 `MixedPainter`，差别只在要不要叠简谱层（`mixedShowJianpuLayer`）。默认 **展开**。
  ...
  **纸与字号两档各记各的**（`app.ts::_sizes` / `jpPaper` / `PersistedSettings`）：切档互不影响，
  导出 PPTX 取的是 **展开档**那一套（`export.ts::pptxPainter`），哪怕屏幕正停在原样档。
  展开档的纸取 `PAGE_RATIOS`（16:9 / 4:3 那几张投影片，1 排版单位 = 1pt，导出 PPTX 要的就是它）
  且**恒分页**；原样档取 `PAPER_SIZES` 的**实际纸张尺寸**（A4 595×842pt、A5、B5、Letter），
  外加一档**长图**（`continuousPage` = 一张连续长纸，宽 `LONG_IMAGE_WIDTH` = 1000、
  高由内容定——**出厂就是这一档**，观感同文本谱的「原版」；那个宽度取自
  `pu/metrics.ts::PRINT.pageWidth`，两者本就该同宽）。长图不是勾选框而是纸张列表里的一档，
  「换纸」与「要不要分页」本来就是同一个决定。**16:9 / 4:3 只给展开档**——那是投影片的比例。
  字号也分两档：展开档标题/词曲各自可调，原样档**只调基础字号**、另两个按出厂比
  （48/28、36/28）派生——那一档换字号是整块缩放，不是只把音符变大。
  分页时标题块排在**第一页顶上**而不是另起标题页（`LayoutOptions.bookHead`），
  首页可用高度按量出来的块高扣减（`firstPageHeadroom`，由 `JinpuPainter.resize` 填，不是给人配的）；
  挂标题块要**另包一层 Group**——`Group.update` 会把子元素归一化到原点、偏移收进 group 自身的
  x/y，往排好的页里直接 `add` 再设 `y`，设的是相对第一条谱行的位置，标题会压到谱面上。
  纸顶那一块**照 500 首重排排**
  （`painter.ts::bookHead` ← `scripts/rebuild.mjs::decorateSong`）：标题居中、**调号拍号在左**
  （`1=♭B ⁴⁄₄`，升降号提到音名之前、小一号并抬高）、**词曲署名右对齐逐行**，
  调号拍号的基线与最后一行署名齐。首调与首拍号从前**根本没画**（编辑器那一路只画
  曲中的转调/转拍号），`.jpwabc` 的调号也一直没写进 `Measure.key`（一律读成 C）。
  展开档的笔画见 `layout/pptxstyle.ts`（底子是 2026-08 重构前的观感，另有几处刻意背离：
  小节线高度用原样档那一份、附点/高音点/低音点三种点同一半径，以及**减时线与旧式八度点
  ...
  上带按 `numberBound("1").top` 往上退 em/8，减时线与弧都贴着数字）；回归 `node scripts/pptx-check.mjs`。
  ...
  **按目标字体的 descent 定文本框底、把音符数字居中到目标宽度的框里**（框心落在排版的
  ...
  成品几何的回归是 `node scripts/pptx-ref-check.mjs`（直接拆开导出的 .pptx 量，与 ppt500 同一套判据）；
  批量转出用 `node scripts/pptx-export.mjs`。

  **展开档的其余几处落点也一律是从成品量回来的目标字体口径**（都在 `layout.ts`）：
  ...
  **乐句排版的「一行几句」走成书那套模式阶梯**（`applybreaks.ts::chooseLineLayout`，
  ...
  （**另起一个 `lyricStack > 0` 的 painter**——展开档的 `buildLine` 走 playData 的完整展开，
  ...
  **数字 ↓ 减时线 ↓ 低音点**这条向下的阶梯走自己的 `jpBelowGap`（1/9 em，按展开档实测的
...
**另一处刻意背离**：`AccidentalStat.update`（`score/score.ts`）在「同一音级在一个小节里再换一次
记号」（先 ♯4 后 ♭4）时，原 Kotlin 直接 `error("")`，整首谱崩在导入这一步。谱面上这写法完全合法
（16《爱心的功课》那串倚音 `5 ♯4 ♭4` 就是），照「新记号相对当前值往哪边走」写出 `b`/`#` 即可，
回到调号本身仍是还原号 `n`。

**无点「1」的绝对音高（各调的音域基准）**：无点的 `1` 落在 **B3(59) … A4(69)** 这一个八度里，
按调号依次往上——C4=60、D4=62、E4=64、F4=65、G4=67、A4=69，到 B 翻回下面的 **B3=59**；
升降号在此基础上 ±1 半音（bB=58、bA=68、#C=bD=61、#F=bG=66、bE=63、bC=59）。
依据是《简谱通用规范》第 23–24 页的各调音域对照表（每调都画满 C2..C6 的五线谱与简谱对照，
逐行看无点的 `1` 落在哪个音）。**只有主音字母是 B 的调（B / bB）整体降一个八度**，A / bA 不降。

实现落在两处、判据必须同源：`score.ts::getBasePitch`（`.jpwabc` / 文本谱侧，按调名字符串判
`step[0] === "B" ? 48 : 60`）与 `jppitch.ts::jpTonicOctaveShift`（MusicXML 侧，按 `fifths`
判 `tonicStep === 6`），后者同时被导入的 `Note.init` 与导出的 `jpPitch` 用，两边互逆。
2026-09 之前这两处**都不对且互不一致**：`getBasePitch` 按字母 `"AB"` 判、把 A/bA 也压到
A3/Ab3，`Note.init` 写死 `fifths ∈ {3,5,-2}`（含了 A、又漏了 bA），同一首 bA 调的谱走
`.jpwabc` 与走 MusicXML 进来能差一个八度。回归用 `node scripts/xml-roundtrip.mjs`
（15 调 × 数字 × 八度点的往返在 `jpPitch` ↔ `Note.init` 之间必须逐项复原）。
...
- **[矢量 PDF 识别](实现/矢量PDF识别.md)**（`src/omr/vector.ts` / `bookprofile.ts` /
  ...
  **「音符」那档不含八度/升降/时值/附点**——别跟位图路那条含八度的 100%（14 首、`scripts/measure-all.mjs`）混口径。
  ...
  **原书自己的错字单列一类**（`glyph_fix` 里人工判得与字典不同的那些），不计入准确率分母
  ——目前 1 处（005 首的「衪」）。可疑字形靠 `scripts/gen-glyphaudit.mjs` 捞：同字同尺寸族内段数
  ...
  **书级内容**（`bookmeta.ts` → 校对.db 八张表 → `bookparts.ts` 排回去）：调号拍号原文
  ...
  补字三条路：`scripts/gen-storyocr.mjs`（**整行**送 OCR + 锚点分段，标点另按墨迹高判；
  句读点不许收在高元素上，否则窄高的括号会被读成「。」）、`scripts/gen-glyphfuzzy.mjs`
  （**形近补字**：同一个字印小一号 `shapeKey` 就变了，用 32×32 签名匹配回字典里已认得的类；
  横向细长条两边都不参与，但**竖的不挡**——窄高的括号宽高比也在 0.25 上下；
  另有上下文规则补生卒年份的连接号）、`scripts/gen-glyphsheet.mjs`（人工确认表，`--from=layout`
  ...
  **B 路 rebuild 的排版口径**（谱面全走排版引擎，整本那一层只管书级装饰）：容量、断句、
  ...
  ——每条判据都记在 [矢量PDF识别](实现/矢量PDF识别.md) 的「成书排版」几节里，**动之前必看**。
  ...
  **装箱**（`scripts/rebuild.mjs` 的 `packMid` / `packAlone`）：短曲**半页起排**接在上一首下面
  ...
  **一首歌不跨同一张纸的正反面**（页数 ≥ 2 的曲子必须起于偶数物理页，差一页时优先
  ...
  末两行要均匀、段落词不出版心、歌词标点不相压）由 **`scripts/line-check.mjs`** 守着：
  `scripts/rebuild.mjs` 顺带写出逐行事实 `pdf-out/rebuild-lines.json`，脚本读它 + drawlist 判**两族**
  ...
  **归类判据一改就要重跑 `scripts/gen-glyphdict.mjs`**（再把上一版的 OCR 标注并回未定类）——
  ...
  `scripts/pdf-mark.mjs` 出**差异标记版 PDF**（红 = 页面读错/多出、黄 = GT 有页面无、橙 = 字形未读出；
  标点不一致是蓝色，**默认不画**、`--punct` 才画——七百多处会把红标淹掉，
  盖在原件上，默认只出有标记的页）；位置由 pdf-diff 写的 `pdf-diff-marks.json` 提供。
  另有 scripts/page-report.mjs 出逐页排版信息（页型/曲目落位/页眉页脚/花边文字框/边框类型，未安置 0），
  scripts/relayout.mjs 从版面规格排回去：outline 模式做对象级核对（全书 spurious 0、unplaced 21/308044），
...
- **[五线谱识别](实现/五线谱识别.md)**（`src/staffomr/` + `src/omr/vectext.ts`）——
  ...
  **`getDocument({disableFontFace:true})` 是字形轮廓可取的前提**，`scripts/node-harness.mjs::openPdf`
  ...
  （标题与歌词读成乱码）靠**拿 GT 自举字形字典**修（`scripts/gen-stafflyrics.mjs`）——
  曲目先按音符序列对上 GT（不依赖字体），再逐字投票；**票要投繁体**，
  且**一个音节一个字形才投**（拉丁词会把字母的类污染成一锅粥）。
  GT 教不到的字（全书只对上一百首）再由 `scripts/gen-staffocr.mjs` **每类送一次 OCR** 兜底，
  ...
  **跨系统连接与声部**（`score.ts` ← `Score::connectSystems`）：逐系统 LCS 对齐谱表签名，
  ...
  对不上拍号就说明那一小节读错了，`node scripts/staff-report.mjs` 打印，全书都能用。
  **和弦层与多声部**（`initChords` / `checkFull` / `splitVoice`）：时值与声部是同一件事
  ——要分声部先得知道每个音从第几拍起，那正是 `checkFull` 分时间列算出来的 offset；
  `toxml` 因此改成按 offset 出（`<forward>`/`<backup>`/`<grace/>`）。
  和弦**先按符干归**（老 `markChords` 判「x 差不到半个符头宽」，认不出二度和弦
  ——那两个符头错开一个符头宽画在符干两侧）。全书小节自检 88.6%→91.4%，
  Opus 那 68 页 SATB 60.1%→67.1%。
  **四处「一个对象画成两半 / 一条判据卡太紧」的要害**（都是拿全书扫出来的，别收紧回去）：
  ...
  对拍 `node scripts/staff-diff.mjs`（基线 `testdata/赞美之泉/staff-baseline.json`），
  口径分档照 500 首那本：只有「音符数与 GT 相差一成以内」的进分母；
  **先确认 GT 读对了再改识别**——读 GT 那一侧数元素的正则**一律写 `<name[ >]`**，
  写 `<name\b`（`scripts/node-harness.mjs`）会命中 `<note-size>`/`<measure-numbering>`/`<part-list>`：
  ...
  **看准确率要看「同一版」那一档**（`scripts/staff-diff.mjs` 单列、`same*` 进基线）：
  ...
  **造字区的汉字（禰/祢）是 JBIG2 贴图**，不在文字层：`openPdf` **必须给 `wasmUrl`**，
  ...
  全书 277 处由 `scripts/gen-staffmasks.mjs` 按位图指纹建表（GT 投票 + 形近 + `maskmanual.json`
  人工确认），定案 272 个；它与 `scripts/gen-stafflyrics.mjs` 要**交替跑**，且后者要把 `masks` 带过去。
  接上之后歌词 92.2% → 95.0%。
  ...
  **全休止与半休止是同一个方块**，按「坐在中线上（半）还是吊在上面一线下（全）」判。
  ...
  接线回归 `node scripts/staff-ui-check.mjs`。**动几何判据前必看那篇。**
- **[位图五线谱识别](实现/位图五线谱识别.md)**（`src/rasteromr/`）——
  **整页乐谱是一张位图**的印刷五线谱 PDF → MusicXML。与上面那条的分界：
  那边文字层齐全、音乐符号是真字符；这边只有像素，文字层剩页眉页脚。
  **下游全部共用**：本模块只把位图变成 `Staff`/`Seg`/`Sym` 装进 `SPage`，
  `page.ts` 往下那一整条一行不改。全链**不起浏览器**——pdf.js 在 Node 下
  对 ImageMask 直接给出逐行 1-bit 打包的字节，解包即可（不复用 `src/omr/decode.ts`，
  那条靠 OffscreenCanvas、还把整页缩到 2000 宽，线距会掉到个位数）。
  **先找谱线、再从五条一组反推线距**（musicpp `findUnit` 取白游程众数的办法
  在这本书上差四倍：符头内部与歌词笔画的小空隙远多于谱线间隙）。
  去谱线判「上下都没墨才抹」，只判游程细会把骑在线上的拍号数字与休止符抹断。
  几何原语靠**游程**分笔画（整幅做连通域会把一行谱连成一块），
  笔画还要判**孤立性**（谱号的中央竖笔、升号的竖笔否则被当成符干抽走，
  符号被自己的笔画切开）；**加线免检**，它只出现在谱线网格的延长线上。
  符头**不查字典、按性质判**（填充率分实心/空心、有没有符干分二分/全音符）——
  与字体无关，而形状签名跟着字体走；符头还要**剪掉粘着的加线**再量，
  剪出来的加线补给 `findLegers`（外露部分只有三四像素，过不了长度闸）。
  谱号/休止/升降/拍号查形状字典，定案三层：**拿矢量路 `glyphmap.json` 那 158 个
  已定案的类当模板**（同一系字体，尺寸逐项吻合；签名要**上下翻**，
  `shapeSig` 的字形坐标 y 向上）、**位置自举**（谱号在谱行开头、调号紧跟其后）、人工小表。
  歌词按位置切出「谱行下方那条带 → verse 行 → 字格」，**整行送 OCR**（序列模型的
  上下文能救回单字），结果按**条的内容指纹**缓存落盘，识别时命中缓存仍不起浏览器；
  **不做形状聚类**（试过，四成多的字格丢在「类里投不出过半票」上）。
  对拍 GT 与识别**都按谱表拆**（`<part>`×`<staff>`，钢琴那种两行谱的 MusicXML
  是逐小节交替写的），分「干净位图 / 真扫描件」两档记账。
  细节按块分篇（取图与谱线 / 符头与加线 / 符干符杠符尾 / 形状字典 / contour 账本与图形符号 /
  歌词 / 度量与跨系统连接 / 现状与待办）在 [实现/位图五线谱识别/](实现/位图五线谱识别/)，
  **动某一块之前先翻对应那篇**。

- **[OMR 简谱识别](实现/OMR-简谱识别.md)**（`src/omr/`，最长的一篇）——图片/PDF → MusicXML。
  全本地一条路：连通域几何 + PaddleOCR PP-OCRv6_small，浏览器离线。14 首 GT 基线：音符/八度/附点/小节/对位/标题/词曲 100%，slur-tie 99.8%、歌词 99.5%。
  回归 `node scripts/measure-all.mjs`、`node scripts/bench-lyrics.mjs`、`node scripts/check-gt.mjs`。
  ...
  **和弦符号**（`src/omr/chordline.ts`）：印在谱行上方的 `Am`/`G/B`/`Gsus4` 也识别出来，
  ...
  变音记号（♯/♭/♮）的形状分类在 `src/omr/accidental.ts`，音符左上角那个与页眉调号里那个**共用同一份判据**；
  文本谱那路是 `src/omr/topu.ts` 直出原文（不过 Score），回归 `node scripts/omr-pu-check.mjs`。
- **[简谱纵向栅格](实现/简谱纵向栅格.md)**（`src/layout/layout.ts` + `layout/upperband.ts`）——
  ...
  （弧高→行高→每页行数→重排），动完必跑 `scripts/rebuild.mjs && scripts/line-check.mjs`。
- **[歌词标点挤压](实现/歌词标点挤压.md)**（`src/common/cjkpunct.ts` + `common/measure.ts` +
  `scripts/punctshape.mjs`）——标点挤压的**两派**都在这里，**全仓唯一一份规则**：
  **全书一律半身式**（开明式，点号占半个字身）——谱面歌词由 `LayoutOptions.punctCompress` 定、
  书级正文由 `scripts/punctshape.mjs::BOOK_MODE` 定；歌词逐字挂音符、标点不占音符格，原书就是半身，
...
- **[乐句排版](实现/乐句排版.md)**（`src/score/phrase.ts`）——工具栏「按乐句重排」的 DP 与代价项
  （行长目标、整句独占一行、断点凭据、段落/副歌分页）。回归 `node scripts/phrase-lines.mjs [曲名子串]`。
  **文本谱也走这套**：断点经 `src/pu/phrase.ts` 换算到 `ScoreLine` 上、由 `src/pu/relayout.ts`
  重切原文的行（展开档与原样档一起变），见 [文本谱 · 按乐句重排](实现/文本谱.md#按乐句重排srcpuphrasets--relayoutts)。
...
- **[ABC 记谱导入](实现/ABC-导入.md)**（`src/abc/`）——abc2xml.py 的全量忠实移植（含 pyparsing /
  etree shim）。改前先核对 python 原文，回归 `node scripts/abc-check.mjs`、`node scripts/abc-shot.mjs`。
- **[文本谱](实现/文本谱.md)**（`src/pu/`）——番茄简谱 / 诗歌本两种**纯文本简谱**的
  原生支持：编辑器直接编原文、专用排版器（原版连续长图 + PPT 版面、四声部并排）、
  导出 MusicXML/`.jpwabc`/MIDI/PPTX、试听逐字高亮。两方言差异与排版判据都在那篇里，
  **动解析或排版常量前必看**。回归 `node scripts/pu-parse-check.mjs`、`node scripts/pu-export-check.mjs`、
  `node scripts/pu-phrase-check.mjs`、`node scripts/pu-shot.mjs`。
- **[混排](实现/混排.md)**（`src/mixed/`）——移植自 C++ 工程 musicpp（`~/proj/musicpp`），
  含测试 musicxml 的位置。改混排行为前先核对 musicpp 原文。
- **[播放速度](实现/播放速度.md)**（`score/timeline.ts`）——谱面 `♩=` × 用户倍率，
  以及它怎么经 `.Title` 的 `Expression` 字段往返 `.jpwabc`。
- **[MusicXML 导出](实现/MusicXML-导出.md)**（`score/musicxmlout.ts`、`musicxmlpatch.ts`、
  ...
  回归 `node scripts/xml-roundtrip.mjs`、`node scripts/omr-export-check.mjs`。

面向仓库读者（非本文件受众）的文档在本目录下：开发、技术栈、进度、macOS 打不开。
...
- Windows 桌面包的 VC++ 运行库随包内置（`src-tauri/tauri.windows.conf.json` +
  `scripts/win-crt.mjs`），DLL 清单改那份 conf；来龙去脉见
  [实现/OMR-简谱识别.md](实现/OMR-简谱识别.md) 的「桌面版（Tauri）也内置 VC++ 运行库」。
