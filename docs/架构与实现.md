...
npm run build && node scripts/pptx-check.mjs                   # 两档版面：简谱档叠排多段 + PPT 档笔画
...
- **设置面板只摆当前模式下真正生效的项**（`editor/dialogs.ts::showOptionsDialog`）。面板是全局的，
  ...
  「每页行数」（写进文档 `.Layout` 段）**只归 PPT 档**——那一档逐段展开、一屏一段，每页放几行是版面决定；
  简谱档按原谱排一遍，行数由内容与纸说了算，人为定死只会把谱挤坏。**没摆出来的项一律不回灌**：
...
- **文本谱也能手动设**（`pu/metrics.ts::applyUserOptions`）：谱面自带的 `FontSize:`/`Margin:` 先生效，
  ...
  换纸给的是实际尺寸（与简谱档共用 `PAPER_SIZES`）：**两档都能换纸、都能改字号，各记各的一套**
  （`puPaper`/`puPptPaper`、`puFontSize`/`puPptFontSize`）——两档的分野只是展开与否，纸张字号是独立设置项。
  PPT 档那份列表没有「长图」（PPT 恒分页），简谱档那份没有「投影片」（那是 PPT 档的 16:9 原生尺寸
...
- **`PPT` / `简谱` 两档的唯一语义是「排版方式」**，不是版式主题、也不是文档格式的分支：
  ...
  **`.jpwabc` 与文本谱（`.pu`）两种格式都要支持这两档，语义完全一致。**
  ...
  `pu/metrics.ts` 尺寸，**没有碰反复展开**，所以文本谱的「PPT 档」目前名不副实
...
- **谱面区的四档排版模式**（`app.ts::ViewMode`，工具条上叫「排版」）是**两组正交状态的组合**，
  ...
  **纸与字号两档各记各的**（`app.ts::_sizes` / `jpPaper` / `PersistedSettings`）：切档互不影响，
  导出 PPTX 取的是 **PPT 档**那一套（`export.ts::pptxPainter`），哪怕屏幕正停在简谱档。
  PPT 档的纸取 `PAGE_RATIOS`（16:9 / 4:3 那几张投影片，1 排版单位 = 1pt，导出 PPTX 要的就是它）
  且**恒分页**；简谱档取 `PAPER_SIZES` 的**实际纸张尺寸**（A4 595×842pt、A5、B5、Letter），
  ...
  「换纸」与「要不要分页」本来就是同一个决定。**16:9 / 4:3 只给 PPT 档**——那是投影片的比例。
  字号也分两档：PPT 档标题/词曲各自可调，简谱档**只调基础字号**、另两个按出厂比
