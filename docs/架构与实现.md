...
- **`PPT` / `简谱` 两档的唯一语义是「排版方式」**，不是版式主题、也不是文档格式的分支：
  **PPT = 展开反复**（一段歌词一遍谱、逐遍成页，投影时一屏一段）、
  **简谱 = 原样排版**（多段歌词叠在同一条谱行下，反复不展开）。
  **`.jpwabc` 与文本谱（`.pu`）两种格式都要支持这两档，语义完全一致。**
  纸张、字号、配色**不属于这两档的分野**——它们是独立的设置项，两档各记各的一套就行。
  照这个口径，简谱那边是对的（`_rebuildPainter` 按档设 `LayoutOptions.lyricStack`：
  > 0 = 叠排不展开）；**文本谱那边还欠着**：`puProfile` 的 print/slide 至今只换整套
  `pu/metrics.ts` 尺寸，**没有碰反复展开**，所以文本谱的「展开档」目前名不副实
  ——要补的是按 verse 展开的能力，不是再去调版式。
- **谱面区的四档排版模式**（`app.ts::ViewMode`，工具条上叫「排版」）是**两组正交状态的组合**，
  不是四个视图：`PPT` / `简谱` 走简谱（或文本谱）排版器，差别只在上面那一条
