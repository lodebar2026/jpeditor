...
npm run build && node scripts/gen-rasterlyrics.mjs             # 位图五线谱：歌词条送 OCR，按内容指纹落盘
node scripts/chorus-symbols.mjs [--one=宁静] [--v]              # 位图五线谱：逐符号（谱号/时值/升降/拍号/松叶/力度）
...
node scripts/raster-prims-check.mjs                            # 位图五线谱：粗加线保符头与正常符杠回归
