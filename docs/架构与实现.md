...
- `src/jianpu/` — 简谱排版两种格式（`.jpwabc` / 文本谱）的**公共那一半**：`profile.ts` 档位、
  `painter.ts` 基类 `JianpuPainter`（`JinpuPainter` 与 `PuPainter` 继承）、`vertical.ts` 纵向分页、
...
- **文本谱也能手动设**（`pu/metrics.ts::applyUserOptions`）：谱面自带的 `FontSize:`/`Margin:` 先生效，
  ...
  换纸给的是实际尺寸（与原样档共用 `PAPER_SIZES`）：**两档都能换纸、都能改字号，各记各的一套**
  （`puPaper`/`puPptPaper`、`puFontSize`/`puPptFontSize`）——两档的分野只是展开与否，纸张字号是独立设置项。
  展开档那份列表没有「长图」（PPT 恒分页），原样档那份没有「投影片」（那是展开档的 16:9 原生尺寸
  `pu/metrics.ts::SLIDE`）。文本谱的字号**只有「基础字号」一项**。前景色走 `pu/painter.ts::setPuInk`——颜色出现在那个文件几十处画笔的默认参数里，
...
- **「展开」/「原样」两档的唯一语义是「排版方式」**，不是版式主题、也不是文档格式的分支：
  ...
  **`.jpwabc` 与文本谱（`.pu`）两种格式语义完全一致**，公共的那一半在 `src/jianpu/`：
  `profile.ts`（档位 `JianpuLayoutMode`；`jpProfile` / `puProfile` 只是两个引擎内部的尺寸档名）、
  `painter.ts`（基类 `JianpuPainter`：切档、展开档的标题页、页脚曲名 + 页码 `0.8 × 纸宽`，页脚只归展开档）、
  `vertical.ts`（纵向分页 `paginate`：只认块的占位盒，两格式共用一份）、
  `expand.ts`（`walkPlay` 按 `PlayItem[]` 逐遍逐小节回调、`versesForPass` 这一遍唱哪几段词；原样档走 `identityPlan`）。
  「这首歌唱成什么样」一律由 `Score.parseRepeatInf()` 推（文本谱先由 `pu/toscore.ts` 补上房号与跳转记号）；
  「怎么放一个小节」各写各的：`.jpwabc` 往 `Line` 里装（`layout.ts::buildLine`），
  文本谱按遍把 AST 重拼一份（`pu/expand.ts`，见 [文本谱笔记](实现/文本谱.md#反复展开展开档)）。
  纸张、字号、配色**不属于这两档的分野**——它们是独立的设置项，两档各记各的一套。
  **换页口径两边一致、只在 `walkPlay` 里判**：`endOfPass`（`.Repeat` 的 `P`、按段数逐遍）与**往回跳**
  （反复、D.C./D.S.）处换页；遍号变了但往前接着唱的（二房后的尾段）不换。源里的强制分页（`$` 分页、`[fenye]`）另算。
