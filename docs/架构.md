...
| A5 | **一个语义模型 `ScoreDoc`，排版引擎只吃它的投影** | 各格式读进同一份模型，引擎输入（简谱 `layout/input.ts`、五线谱版面态 `StaffLayout`）由它投影、不另存语义；MusicXML 存回时没改过给原文，改过整份重写（不再 patch）。**MusicXML 独有的表层不进模型**：`fromxml` 把模型对象绑到原节点（`model/xmlsurface.ts` 旁表），五线谱引擎经查询函数现读，写出端按「写出端管的」之外通用回填 |
...
flowchart TB
  ...
  subgraph STYLE["样式（横切）style/"]
    ...
    sheet -- 适配器 --> rulers["LayoutOptions │ JianpuMetrics<br/>BookStyle │ MixedOptions"]
  end
...
全项目**唯一的排版器**：按请求排版并提交结果，管页面渲染（DOM 映射）、点选与播放高亮。请求三种——
简谱引擎（`JianpuPaintRequest`：原样 / 展开，成书、帮助、PPTX、量宽也走它）、原样文档（`DocumentPaintRequest`：
文本谱与多声部 123/ABC、MusicXML 的原样档，`layout/original/`）、五线谱 / 混排（`StaffPaintRequest`，异步、带版本核验）。
...
两路页面树的类名差别在排版器里消化，编辑器不判断「是哪一路画的」。
简谱引擎与原样文档排版都走同步 `loadSync`（构造时 SMuFL 字体已就绪，不需要等资源），只有五线谱／混排
（`StaffPaintRequest`）是异步、带版本核验；目前没有路径需要为其余部分等待资源，真要等资源时再把
重排链（`App.reload` 与各格式适配器的 `reload`）Promise 化。
...
| `.123` | `parse123` → `ScoreDoc` →（`pu/slots.ts` 排版行视图）→ `jianpuInputOfDoc` → `ScorePainter`（原样单声部／展开；多声部原样走原样文档布局 `layout/original/`） |
...
| `src/score/` | 演唱顺序（`playorder.ts`）+ 乐句断句（`phrase.ts`/`applybreaks.ts`）+ 试听时间线/MIDI + 调号音高工具（`jppitch.ts`） | [排版-简谱引擎](模块/排版-简谱引擎.md) · [播放](模块/播放.md) |
...
| `src/editor/` | 编辑器控制器、**源格式适配器表**、可视化编辑、导出、播放、OMR 控制器 | [编辑器](模块/编辑器.md) · [可视化编辑](模块/可视化编辑.md) · [播放](模块/播放.md) · [导出](模块/导出.md) |
...
叠上用户层（编辑器面板）或歌本样式表（`.ss`；成书的 `BookStyle` 也由它算出，没有 json），经 `cascade.ts::computeStyle` 按上下文
...
| `JianpuMetrics` | `layout/original/metrics.ts` | `style/original.ts`（内置方言表 `style/books/original*.ss`） | 原样文档布局：文本谱、MusicXML、多声部 123/ABC 的原样档（从原版实测得来） |
