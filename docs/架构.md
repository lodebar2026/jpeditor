...
npm run build:cli      # 不碰 DOM 的模块打成 Node ESM（dist-cli/），给无头脚本 import
...
- ANTLR 生成码在 `src/jpword/parser/`，**勿手改**；重生成步骤见 [源格式-jpwabc](模块/源格式-jpwabc.md)
...
- **回归脚本**全在 `scripts/`，一律从仓库根跑（`node scripts/xxx.mjs`），用法在各自文件头，**各模块页的「回归」一节是它自己的那几条**。
  要浏览器的先 `npm run build`，只 import `dist-cli/` 的先 `npm run build:cli`；语料路径用环境变量
  `HYMN500=<500首语料根>`、`PU_CORPUS=<文本谱语料根>`（缺省时跳过或只跑 `testdata/`）。
  浏览器那一路用 Playwright `channel: "msedge"` 驱动本地 Edge，引导（MIME 表 / serve dist / 起 Edge / 遍历 testdata 夹具 / 读 GT）
  共用 `scripts/harness.mjs`，**加新资源类型只改那里的 MIME 表**；不起浏览器的在 `scripts/node-harness.mjs`；
  `window.__app` 运行时暴露（`src/main.ts`）
