...
- **谱速来源与往返**：MusicXML `<sound tempo>`（OMR 页眉的 `♩=76`、ABC 的 `Q:` 都吐这个）
  → `score/musicxml.ts::parseSound` → `PlayData.tempo`（`ScoreDoc` 那侧：`Direction.sound.tempo`，
  小节级 `<sound>` 记成 `type: "sound"`，`playdoc.ts::tempoOfDoc` 取）→ `model/tojpw.ts` 写 `.Title` 的
  `Expression = {♩=76}` → `jpwimport.ts` / `fromjpw.ts`（`Song.tempos`）读回。
