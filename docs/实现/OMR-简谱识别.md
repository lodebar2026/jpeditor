...
- **谱行上方带**：第 i 行的「下方带」就是第 i+1 行的「上方带」，本是同一条带；唯独第 0 行的上方带
  ...
  故 `recognizeLyrics` 的行循环从 **i=-1** 起，那条带从第 0 行上缘往上量 **numH*2.5**。
  这条带只走和弦通道：**不产段落标记、不产跳转记号、不进歌词装配**——它本不在旧管线的视野里，
  让它产出别的就等于凭空改动既有行为（实测：放它捞段落标记，「沧海一声笑」六段词的歌词从 99.4%
  掉到 80.8%——噪声命中 `SECTION_MARK_RE`，给首行安了个假段落起点，乐句排版据此硬换行）。
  **2.5 这个倍数别放大**：放到 3.5 会把「沧海一声笑」页眉里的表情记号卷进来（Expression 字段随之丢失，
  歌词同样塌到 80.8%）。代价是够不着「再次将我更新」第一谱行的 5 个和弦——它与谱行之间还隔着一排
  圆滑线，距顶 2.54 个字号，差 1px 就在带外。这 5 个是已知漏检，宁可漏也不动页眉。
...
**和弦基线**：备了和弦 GT 的三首——主祢真伟大 **95.3%**（43/43 全部落位正确，两处 OCR 把 F 读成 E）、
哦愿我有千万舌头 **100%**（16/16）、再次将我更新 33/38（第一谱行 5 个漏检，其余全对；该曲无 `.jpwabc`
GT，不进 `measure-all`）。**和弦 GT 的载体是 `testdata/<曲名>/gt.tomato.pu`** —— 一份人工核对过的
番茄文本谱原文（`.jpwabc` 装不下和弦，只能另置载体）；`measure-all.mjs` 的「和弦」档比对
`"hx:X"` 序列，只对备了这份文件的歌谱计分。回归还有 `node omr-pu-check.mjs`（逐音符和弦往返无损，
两方言各跑一遍）与 `node omr-export-check.mjs`（`<harmony>` 列入 patch 保全清单——和弦只活在底本里，
patch 顺手删掉它，jpwabc 那边看不出任何异样）。
