...
**减时线的连断（文本谱的 `^`）**：文本谱把相邻两个带减时线的音符自动连成一条线、满一拍才断
（`pu/layout.ts::computeUnderlines` 按 `floor(beat)` 分组）。多数地方按拍分组就与谱面一致、
不必多写记号；只有**小节里出现过切分音**（音符从非整拍起、又跨过整拍线）之后拍位整个错开，
读谱的一方最容易在「减时线层数变了」的那个交界上连错——那里才写 `^` 强制断开。
714《我说算了吧》全曲恰好两处这样的切分（`2g/ 2g` 与 `7/ 7` 领起的两小节），与人工订的谱面
完全对上。先试过「凡跨整拍都写」，全曲写出五处、多了三处；也试过按**源图**判两音符的第一层
减时线是不是同一条横块——分不开：实测同组内两段横线的间隙 10px、该断开处 15px，几何上没有界。
