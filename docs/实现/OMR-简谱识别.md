...
——每小节末尾的长音全变短。补一条细长条通道：`w ≥ 0.4 字号 && w ≥ 3h`。
...
**④ 减时线与低八度点粘成一块要切开——只在干净谱面上切。** 《竭力保守》第 6 行 `2̲ 1̳ 6̳` 的第二条
减时线与那个低八度点 4-连通粘成 207×15 一块，过不了「横线要够扁」（h ≤ 0.32 字号）那道门，
**线与点一起消失**：那一拍时值翻倍、音高还高了八度。`splitMergedUnderlineDot` 按行墨宽切
（线那几行墨宽≈整块宽，点那几行只有点径）。
