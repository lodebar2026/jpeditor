...
**不依赖大小写正确**（OCR 常把 C 读成 c）；剔后该谱行剩余 verse 重新编号。判定前先**剥掉方括号里的编者注**
...
- **行长约束是软惩罚**：可断点稀疏时（副歌里弧线跨小节连成一片）硬禁会无解。除小节数外还限
  **每行格数** `MAX_CELLS=25`（格 = 音符 1 + 每根增时线 `DASH_W=0.7`——增时线渲染得比数字窄，
  ...
  能在 5 小节（`STRICT_SENTENCE_MEAS`）/ 30 格内完整收于句号的句子允许稍宽，并惩罚句号前的提前拆行；
  更长的句子仍可在逗号或音乐信号处换行。**断点自身毫无乐句收尾凭据时**（无标点、非长音、无延长号——
  只是正好到了一条小节线，哪怕它是重复段边界），这个上限放到 `MAX_SENTENCE_MEAS=6`：在那里截断句子
  最没道理（「立定心志」末两行 `…涌流在我心，我立定 | 心志，走在主圣洁光中。` 因此并成一行 6 节 26.5 格，
  实测正好放进版心）；而「基督更美」的 `…从开始到最终，| 讲述救赎…` 断在逗号上，仍按 5 小节上限正常分行。段落标题若落在新小节、而上一句的 1~2 个收尾音占用该小节开头，则段界移到句号之后。
...
回归：`node phrase-lines.mjs [歌谱名子串]`（走 OMR→导入→`setPhraseLayout(true)`，逐行打印
「小节数/格数/W1 歌词/音符」；musicxml 结果缓存在 scratchpad，改 phrase.ts 后重跑很快，
`PHRASE_FRESH=1` 强制重识别）。当前 13 首基线：**每行都落在 3~5 节 / 11~25 格**（主祢真伟大 9 行、
日光之下 6 行、耶稣普治 4 行…；从小节中间起头的行会显示 2 节但格数够）。
出现「整段一行」「1 小节一行」都是退化。
