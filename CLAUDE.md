...
- **[矢量 PDF 识别](docs/实现/矢量PDF识别.md)**（`src/omr/vector.ts` / `bookprofile.ts` /
  ...
  （**整行合成一个 path 的按整行送行识别**，收多字符结果）——字典覆盖 97.9%。
  568 首基线：音符 99.9%、歌词 98.8%、标题 99.2%、和弦 97.8%、圆滑线 96.5%、调号 97.5%、拍号 99.6%、词曲署名 95.2%
  （调号/拍号剩下的差异几乎全是 GT 按主音和弦订正过、与谱面不同调，真没读到的只剩拍号 1 首）
  （后四项以 musicxml 为基准——.jpwabc 装不下和弦，slur/tie/调号拍号也是 musicxml 才全），
  全书未归类对象 0/365694，内容完全一致（音符+歌词+标题）424/568。
