...
- **[五线谱识别](docs/实现/五线谱识别.md)**（`src/staffomr/` + `src/omr/vectext.ts`）——
  ...
  **四处「一个对象画成两半 / 一条判据卡太紧」的要害**（都是拿全书扫出来的，别收紧回去）：
  内层符杠往符头方向叠、离符干末端一格开外，`beamConnect` 要**朝符头一侧**放宽到两格
  （不然十六分读成八分，全书 158 处）；每条弧从极点处劈成两个路径对象，
  `mergeArcHalves` 要按「端点重合 + 接口是极点」并回一条（不然连音线整批读成圆滑线）；
  弧端离符头的纵向窗口是**三格**不是一格（朝上的弧画在符杠之上）；
  房号罩着哪行谱按「**竖钩底贴着谱表顶**」判、钩不限长（横线的高度随上方有没有歌词浮动）。
  歌词那一侧另有两条：中文歌词行**既无连字符也无延长线**，要拿本页已认定的歌词
  自校准出字体/字号/带宽再补一道锚（不然整行丢掉）；每个字旁边那个 ToUnicode 为 `!`、
  整字见方的空轮廓字形是**全角空格**，按判据认（不是靠形近补字歪打正着标成「1」）。
  对拍 `node staff-diff.mjs`（基线 `testdata/赞美之泉/staff-baseline.json`），
  口径分档照 500 首那本：只有「音符数与 GT 相差一成以内」的进分母；
  **先确认 GT 读对了再改识别**——读 GT 那一侧数元素的正则**一律写 `<name[ >]`**，
  写 `<name\b`（`node-harness.mjs`）会命中 `<note-size>`/`<measure-numbering>`/`<part-list>`：
  `xmlStaffTypes` 漏了数字让 `16th`/`32nd` 全读成 `?`（白记 516 处时值错），
  `xmlMeasureCount` 多数了 `<measure-numbering>` 让每首凭空多一个小节
  （**全书 64/77 首那个整齐的「小节数差 −1」全是它**，改完逐首对上 2/77 → 65/77）；
  GT 的 `<bar-style>none</bar-style>`（不可见小节线，全书 141 处）要把两个 `<measure>`
  **并回一个**，也不能算进「结构性小节线」。
  识别侧同理：**行首行末切出的空条不是小节**（行首那一段只有谱号调号拍号、
  行末还剩一小截），全书凭空多出 120 个；行首那条删掉时它的样式要落到
  新首小节的左端（`Bar.leftStyle`，`|:` 是 `heavy-light`）。
  **看准确率要看「同一版」那一档**（`staff-diff.mjs` 单列、`same*` 进基线）：
  「数目相当」里混着只印了英文歌词版的曲子（GT 是中文版，编配本就有出入），
  拿歌词 ≥80% 把同一份谱挑出来才是识别的账——同一版 61 首音符 98.6%、时值 97.3%、
  弧线 96.0%，没有中文歌词那 20 首只有 94.6%。含升降的音高档遇到移调版要
  **按新调重算升降**（`shiftPitch`），原样带着平移会让 272 首掉到 46.2%。
  **弧的宽度下限只能按「半条弧」量**（`findSlurs`，每条弧从极点劈成左右两半各画一个对象）：
  取 1.2 格会把两个音符之间那种短连音线整批挡掉，0.9 格之后全书弧数与 GT 分毫不差（423/423）。
  **歌词是配对的仲裁**：真配对最低 62%、错配最高 6%，分档极干净——低于 0.3 就否决这一对
  （两边都还留在池子里让别人认领），≥ 0.45 可直接当第三条配对通道，
  对上的那一段再加 0.05 分把**中文版页面**从英文版手里抢回来。这道闸只在
  谱面**汉字占比过三成**时才开，否则会误伤英文版页面（标题加版权行就凑够十个汉字）。
